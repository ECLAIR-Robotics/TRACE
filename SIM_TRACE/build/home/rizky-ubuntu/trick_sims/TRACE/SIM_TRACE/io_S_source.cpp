...
void init_attrTRACESimObject() {
    ...
    trick_MM->add_attr_info(std::string(attrTRACESimObject[3].type_name) , &attrTRACESimObject[3], __FILE__ , __LINE__ ) ;
...
void init_attrTRACESimObject_c_intf() {
    init_attrTRACESimObject() ;
}

size_t io_src_sizeof_TRACESimObject() {
    return sizeof(TRACESimObject) ;
}

void* io_src_allocate_TRACESimObject(int num) {
    TRACESimObject* temp = (TRACESimObject*)calloc(num, sizeof(TRACESimObject));
    for (int ii = 0; ii < num; ++ii) {
        new(&temp[ii]) TRACESimObject();
    }
    return (void*)temp;
}

void io_src_destruct_TRACESimObject(void* address __attribute__((unused)), int num __attribute__((unused))) {
TRACESimObject* temp = (TRACESimObject*)address;
    for (int ii = 0; ii < num; ++ii) {
        temp[ii].~TRACESimObject();
    }
}

void io_src_delete_TRACESimObject(void* address) {
    delete (TRACESimObject*)address;
}

} //extern "C"

struct UnitsMapTRACESimObject {
    UnitsMapTRACESimObject() {
        Trick::UnitsMap* units_map_ptr = Trick::UnitsMap::units_map();
        units_map_ptr->add_param("TRACESimObject_name", "1") ;
        units_map_ptr->add_param("TRACESimObject_id", "1") ;
        units_map_ptr->add_param("TRACESimObject_object_disabled", "1") ;
        units_map_ptr->add_param("TRACESimObject_ball", "1") ;
    }
} umTRACESimObject;
